...
export default function EditGalleryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/gallery/${slug}`)
      ...
      .catch(() => setToast({ message: t('Failed to load', 'بارگذاری ناموفق'), type: 'error' }))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setToast({ message: t('Saved', 'ذخیره شد'), type: 'success' })
      } else {
        const d = await res.json() as { error?: string }
        setToast({ message: d.error ?? t('Failed to save', 'ذخیره ناموفق'), type: 'error' })
      }
    } catch {
  ...
  const handleDelete = async () => {
    if (!confirm(t('Delete this product?', 'این محصول حذف شود؟'))) return
    setDeleting(true)
    try {
      ...
      if (res.ok) {
        ...
        setToast({ message: t('Delete failed', 'حذف ناموفق'), type: 'error' })
      }
    } catch {
      setToast({ message: t('Network error', 'خطای شبکه'), type: 'error' })
    } finally {
    ...
    <div className="p-8 max-w-3xl">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-[#f4f1ea] text-2xl font-semibold">{nameEn || slug}</h1>
        </div>
        <button
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input value={code} onChange={e => setCode(e.target.value)} className={`${inputCls} font-mono text-xs`} />
          </div>
          <div>
        ...
        <div className="border-t border-[#c6a25f]/20 pt-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-[#f4f1ea]/80 text-sm uppercase tracking-widest">{t('Images', 'تصاویر')} ({images.length})</h2>
            <button
                  ...
                  <div key={img} className={`relative group rounded overflow-hidden border-2 transition-colors ${isThumb ? 'border-[#c6a25f]' : 'border-[#c6a25f]/10'}`}>
                    ...
                    {isThumb && (
                      <div className="absolute top-1.5 left-1.5 bg-[#c6a25f] rounded px-1.5 py-0.5 flex items-center gap-1">
                        ...
                        <span className="text-[#0c1626] text-[10px] font-bold">{t('Thumb', 'شاخص')}</span>
                      </div>
                    )}
                    <div className="absolute inset-0 bg-[#0c1626]/75 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                      {!isThumb && (
                        <button
                          ...
                          title={t('Set as thumbnail', 'تنظیم به عنوان شاخص')}
                        >
                      ...
                      <button
                        onClick={() => {
                          ...
                          if (thumbnail === img) setThumbnail(images.find(u => u !== img) ?? '')
                        }}

      ...
      <main>
        ...
        <section className="bg-[#0c1626] pt-36 pb-12">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
            ...
            <h1 className="font-display text-4xl sm:text-5xl font-bold text-white mb-3">{cat.nameEn}</h1>
          ...
          <section className="bg-[#fbfaf6]">
            <div className="grid grid-cols-1 sm:grid-cols-2">
              {subcategories.map((sub) => (
                <CategoryCard key={sub.slug} category={sub} locale="en" />
              ))}
            </div>
          </section>
        ) : (
...
async function ProductListingWrapper({
  ...
  return (
    <section className="bg-[#fbfaf6] py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
        <Suspense>
          <ProductFilter locale="en" colors={colors} finishes={finishes} />
        </Suspense>
        {filtered.length === 0 ? (
